...
__end_caculate:
        ...
        if (icon_p->size == 0) {
            ...
            if ((svi.lcd_w == 454) && (r0 > 0)) {
                elm->css.width = 2 * r1 * 10000 / svi.lcd_w;
                elm->css.height = 2 * r1 * 10000 / svi.lcd_h;

                elm->css.left = cx * 10000 / svi.lcd_w - layout_elm->css.left;
                elm->css.top = cy * 10000 / svi.lcd_h - layout_elm->css.top;
...
static int pic_sys_reboot_and_shutdown_ontouch(void *_ctrl, struct element_touch_event *e)
{
    ...
    case ELM_EVENT_TOUCH_UP:
        ...
        switch (pic->elm.id) {
        case PIC_SYS_REBOOT:
            extern int lcd_power_ctrl(u8 on);
            lcd_power_ctrl(false);
